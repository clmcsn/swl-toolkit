...
def make_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    port_number = df['clusters'].unique()[0]
    area = DDEFS.AREA[DDEFS.AREA['clusters'] == port_number]['area'].values[0]
    sdf = pd.DataFrame({
        "app": [df['app'].unique()[0]],
        "clusters": [port_number],
...
def get_dataframe(path: str, dir_name: str) -> pd.DataFrame:
    ...
    df = df.sort_values(by=['clusters', 'workload_size']).reset_index(drop=True)
