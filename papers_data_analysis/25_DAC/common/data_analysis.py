...
def add_flops(df: pd.DataFrame, app: str) -> pd.DataFrame:
    ...
    match app:
        ...
        case "conv2d":
            df['flops'] = (df['out_channels'] * df['in_channels'] * df['workload_size_x'] *
                           df['workload_size_y'] * 9)
        case "sgemm":
...
def process_workload_size(df_merged: pd.DataFrame, app) -> pd.DataFrame:
    ...
    if app == "sgemv":
        df_merged['workload_size'] = df_merged['workload_size_x']
    if "vlen" in df_merged.columns:
        df_merged['workload_size'] = df_merged['vlen']
...
def make_ratios(df_merged: pd.DataFrame, app: str) -> pd.DataFrame:
    ...
    if app == "aggr" or app == "gcn-aggr":  # make different case
        for ds in df_merged['dataset'].unique():
            for ll in df_merged['vlen'].unique():
                base_df = df_merged[(df_merged['dataset'] == ds) &
                                    (df_merged['vlen'] == ll) &
                                    (df_merged['kernel'] == CDEFS.BASE_KERNELS[app])]
                base_instrs = base_df['instrs'].values[0]
                base_cycles = base_df['cycles'].values[0]
                df_merged.loc[(df_merged['dataset'] == ds)
                              & (df_merged['vlen'] == ll)
                              & (df_merged['kernel'] != CDEFS.BASE_KERNELS[app]),
...
def make_avg(df: pd.DataFrame, metric: str) -> pd.DataFrame:
    ...
    avg = df[metric].mean()
    df = pd.concat([df, pd.DataFrame({'app': ['mean'], metric: [avg]})])
    return df
