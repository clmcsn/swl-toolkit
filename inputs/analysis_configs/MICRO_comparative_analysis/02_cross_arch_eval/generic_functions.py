...
def make_summary_df(df):
    ...
    for t in df['threads'].unique():
        area = sf.area[sf.area['threads'] == t]['area'].values[0]
        sum_df = pd.concat([pd.DataFrame({
                "app" :                     [df['app'].unique()[0]],
                "threads" :                 [t],
                "cycles" :                  [df[df['threads'] == t]['cycles'].mean()],
                "cycle_area" :              [df[df['threads'] == t]['cycles'].mean()*area],
                "ssr_stalls" :              [df[df['threads'] == t]['ssr_stalls'].mean()],
                "dcache_bank_stalls" :      [df[df['threads'] == t]['dcache_bank_stalls'].mean()],
                "dcache_bank_utilization" : [df[df['threads'] == t]['dcache_bank_utilization'].mean()]
        }), sum_df])
