...
#####################################################################
# dirs
base_dir = "/users/micas/gsarda/vortex/vortex-KUL-fork/scripts/outputs/04-SWIFT-COMP2/"
...
for m in ['cycles', 'ssr_stalls', 'cycle_area', 'dcache_bank_utilization', 'dcache_bank_stalls']:
    g = sns.catplot(x="app", y=m, hue="threads", data=sum_df, kind="bar", height=8.45/3*cm*scale, aspect=aspectR, width=0.6, legend=False)
    ...
    if m == 'dcache_bank_utilization':
        plt.ylim(0, 100)
    if m == 'dcache_bank_stalls':
        plt.legend(title='Threads', loc='upper center', bbox_to_anchor=(0.5, 0.5), ncol=4, prop=fontproperties,title_fontproperties=fontproperties)
