...
def make_summary_df(df):
    sum_df = pd.DataFrame()
    for t in df['clusters'].unique():
        area = sf.area[sf.area['clusters'] == t]['area'].values[0]
        sum_df = pd.concat([pd.DataFrame({
                "app" :                     [df['app'].unique()[0]],
                "clusters" :                 [t],
                "cycles" :                  [df[df['clusters'] == t]['cycles'].mean()],
                "cycle_area" :              [df[df['clusters'] == t]['cycles'].mean()*area],
                "ssr_stalls" :              [df[df['clusters'] == t]['ssr_stalls'].mean()],
                "dcache_bank_stalls" :      [df[df['clusters'] == t]['dcache_bank_stalls'].mean()],
                "dcache_bank_utilization" : [df[df['clusters'] == t]['dcache_bank_utilization'].mean()]
        }), sum_df])
