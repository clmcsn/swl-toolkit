...
df['cycle_ratio'] = 1
#plot cycle ratio knn/others
for ws in df['workload_size'].unique():
    knn_base_cycles = df[(df['kernel'] == 'knn-asm') & (df['workload_size'] == ws)]['cycles'].values[0]
    df.loc[(df['kernel'] != 'knn-asm') & (df['workload_size'] == ws), 'cycle_ratio'] = knn_base_cycles / df[(df['kernel'] != 'knn-asm') & (df['workload_size'] == ws)]['cycles'] 
...
df['instr_ratio'] = 1
for ws in df['workload_size'].unique():
    knn_base_instrs = df[(df['kernel'] == 'knn-asm') & (df['workload_size'] == ws)]['instrs'].values[0]
    df.loc[(df['kernel'] != 'knn-asm') & (df['workload_size'] == ws), 'instr_ratio'] =  knn_base_instrs / df[(df['kernel'] != 'knn-asm') & (df['workload_size'] == ws)]['instrs']
