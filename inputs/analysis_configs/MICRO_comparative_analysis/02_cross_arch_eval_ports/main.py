...
#####################################################################
# dirs
base_dir = "/users/micas/gsarda/vortex/vortex-KUL-fork/scripts/outputs/04-SWIFT-COMP-PORTS/"
...
plot_labels = {
    'cycles': 'Cycles',
    'ssr_stalls': 'Norm. SSL Stalls',
    'cycle_area': 'Cycle-Area Product',
    'dcache_bank_utilization': 'Cache Bank Utilization',
    'dcache_bank_stalls': 'Dcache Bank Stalls'
}

sum_df = sum_df[sum_df['app'].isin(['saxpy', 'sgemm', 'sfilter', 'knn'])]

for m in ['cycles', 'ssr_stalls', 'cycle_area', 'dcache_bank_utilization', 'dcache_bank_stalls']:
    g = sns.catplot(x="app", y=m, hue="dcache_ports", data=sum_df, kind="bar", height=8.45/3*cm*scale, aspect=aspectR, width=0.6, legend=False)
    ...
    if m == 'cycles':
        plt.ylim(0.5, 1.1)
    if m == 'dcache_bank_utilization':
        plt.legend(title='Ports', loc='upper center', bbox_to_anchor=(0.5, 0.5), ncol=4, prop=fontproperties,title_fontproperties=fontproperties)
