...
df_file = "/vx/scripts/outputs/MICRO-COMP-saxpy/dataframe.feather"
output_dir = "/vx/scripts/outputs/MICRO-COMP-saxpy/comparative_analysis/"

df  = pd.read_feather(df_file)
cdf = df.groupby('workload_size').apply(lambda x: (x.loc[x['kernel'] == 'saxpy', 'cycles'].iloc[0] - x.loc[x['kernel'] == 'saxpy-limbo', 'cycles'].iloc[0]) * 100 / x.loc[x['kernel'] == 'saxpy', 'cycles'].iloc[0])
cdf = cdf.reset_index()
cdf.columns = ['workload_size', 'cycles']
ndf = df.groupby('workload_size').apply(
        lambda x: (x.loc[x['kernel'] == 'saxpy-limbo', 'dcache_read_hit_ratio'].iloc[0])).reset_index()
...
# Making instrs plot
cdf = df.groupby('workload_size').apply(lambda x: (x.loc[x['kernel'] == 'saxpy', 'instrs'].iloc[0] - x.loc[x['kernel'] == 'saxpy-limbo', 'instrs'].iloc[0]) * 100 / x.loc[x['kernel'] == 'saxpy', 'instrs'].iloc[0])
cdf = cdf.reset_index()
